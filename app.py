...
def userHome():
    if session.get('user'):
        ...
        if data[0][4] == 1:
            ...
            query="SELECT mo.orderID, mo.OrderDate, mo.Total_price, m.title, mo.number, m.price, m.image FROM movie_order AS mo JOIN \
...
def allMovies():
    ...
    if request.method == "POST":
        ...
        cursor.execute("SELECT * FROM movie as m WHERE m.title LIKE %s", (searchbox))
        data = cursor.fetchall()
        conn.commit()
        conn.close()
        cursor.close()
        return render_template('listMovies.html', data=data)
    else:
        cursor.execute("SELECT * FROM movie")
